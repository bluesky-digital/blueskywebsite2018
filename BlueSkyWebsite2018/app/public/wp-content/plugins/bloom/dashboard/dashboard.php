...
class ET_Dashboard {
	...
	public static function generate_modal_warning( $message = '', $ok_link = '#', $hide_close = false, $ok_text = '', $custom_button_text = '', $custom_button_link = '#', $custom_button_class = '' ) {
		...
		$ok_link = isset( $_POST[ 'ok_link' ] ) ? sanitize_text_field( $_POST[ 'ok_link' ] ) : $ok_link;
		...
		$custom_button_link = isset( $_POST[ 'custom_button_link' ] ) ? sanitize_text_field( $_POST[ 'custom_button_link' ] ) : $custom_button_link;
	...
	function generate_options_page( $sub_array = '' ) {
			...
			foreach ( $dashboard_sections[ 'header' ][ 'contents' ] as $key => $value ) {
				printf(
					'<li class="et_dashboard_tab_content_header_%1$s">
						<a href="#tab_et_dashboard_tab_content_header_%1$s" id="et_dashboard_tab_content_header_%1$s" class="et_dashboard_icon_header_%1$s et_dashboard_icon" title="%2$s">
							<span></span>
						</a>
					</li>',
					esc_attr( $key ),
							...
							switch( $option[ 'type' ] ) {
								...
								case 'input_field' :
									printf(
										...
										'number' == $option[ 'subtype' ] ? '0' : esc_attr( $option[ 'placeholder' ] ),
									...
									foreach ( $post_types as $post_type => $id ){
										...
										printf( '
											...
											( 'post_cats' === $option['subtype'] && isset( $option['include_custom'] ) )
												? esc_attr( $id ) . esc_attr__( ' ( post )', 'bloom' )
												: esc_attr( $id ),
		...
		printf(
			...
			esc_html( $this->save_button_text ),
			'' !== $sub_array
				? sprintf( 'data-subtitle="%1$s"', esc_attr( $sub_array ) )
				: '',
	 		esc_attr( apply_filters( 'et_' . $this->plugin_name . '_save_button_class', '' ) )
		);
						...
						switch( $option[ 'type' ] ) {
							case 'import_export' :
								printf(
									...
									<div class="et_dashboard_import_form et_dashboard_row">
										...
										<form method="post">
											...
											<p>',
									esc_html( $option[ 'title' ] ),
									esc_html__( sprintf( 'You can either export your %1$s Settings or import settings from another install of %1$s below.', esc_html( ucfirst( $this->plugin_name ) ) ), 'et_dashboard' ),
								...
								printf(
									...
									esc_attr( $this->_options_pagename ),
									'toplevel_page' == $this->top_level_page ? 'admin' : esc_attr( $this->top_level_page )
								);
